...
class CustomCardSwiper extends StatefulWidget {
  ...
  @override
  _CustomCardSwiperState createState() => _CustomCardSwiperState();
...
class _CustomCardSwiperState extends State<CustomCardSwiper>
  ...
  List<Widget> _buildCards(BuildContext context) {
    ...
    for (int i = _currentFront;
        i < _currentFront + widget._stackNum;
        i++) {
  ...
  @override
  void didUpdateWidget(covariant CustomCardSwiper oldWidget) {
  ...
  Widget build(BuildContext context) {
    widget.cardController
        .addListener((trigger) => triggerSwap(trigger));
...
class CardAnimation {
  static Animation<Alignment> frontCardAlign(
    ...
    if (_CustomCardSwiperState._trigger == TriggerDirection.none) {
      endX = beginAlign.x > 0
          ? (beginAlign.x > swipeEdge
              ? beginAlign.x + 10.0
              : baseAlign.x)
          : (beginAlign.x < -swipeEdge
              ? beginAlign.x - 10.0
    ...
    else if (_CustomCardSwiperState._trigger == TriggerDirection.up ||
        _CustomCardSwiperState._trigger == TriggerDirection.down) {
      var beginY =
          _CustomCardSwiperState._trigger == TriggerDirection.up
              ? -10
              : 10;
